...
export const ColorPicker: FunctionComponent<ColorPickerProps> = (props: ColorPickerProps) => {
  ...
  useEffect(() => {
    ...
    if (!colorPicker.current) {
      colorPicker.current = iro.ColorPicker(el.current, {color: props.color, layout: [{component: iro.ui.Wheel}], width: 400, handleRadius: 12});

      if (!colorPicker.current) {
        return;
      }

      colorPicker.current.on("color:change", (color: {hexString: string}) => { throttledOnChange(color.hexString) });
